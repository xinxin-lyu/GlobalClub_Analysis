...
******* Main variables: L, Contribution %
******* Main variables: LG, Join
******* Main variables: LG, Global Contribution %
******* Main variables: LG, Global Contribution %, members only

************** Independent variable: treatment variables 
************** Purpose: establish a comparison for beween-subject treatments
************** Regression include R1 and R2+
preserve

local rounds = 10
keep if subsessionperiod<=`rounds'
local filename "$outputfolder\05-RegressionAll-v4-Main-Treatments-`rounds'.xls"

// local filename "$outputfolder\05-RegressionAll-v4-Main-Treatments-20.xls"
...
* L,  Contribution %
xi: xtreg round1_totCtr_share hetero `all_controls' if  WithGlobal==0 &  subsessionperiod==1 , re cluster(sessioncode)
outreg2 using `filename' ,  replace `settings'  ctitle("L Contribution % in round 1")

xi: xtreg share_totalContr hetero `all_controls'  if  WithGlobal==0  , re cluster(sessioncode)
outreg2 using `filename' ,  append `settings'  ctitle("Contribution % in round t+1")

* LG, join 
xi: xtreg round1_join `treatment_related' `all_controls' if  WithGlobal==1 &  subsessionperiod==1 , re cluster(sessioncode)
outreg2 using `filename' ,  append `settings'  ctitle("LG join in round 1")

xi: xtreg playerjoin_club `treatment_related' `all_controls'  if  WithGlobal==1  , re cluster(sessioncode)
outreg2 using `filename' ,  append `settings'  ctitle("LG join in round t+1")

* LG, Global contribution % 
xi: xtreg globalCtr_share `treatment_related' `all_controls' if  WithGlobal==1 &  subsessionperiod==1 , re cluster(sessioncode)
outreg2 using `filename' ,  append `settings'  ctitle("LG Global contribution % in round 1")

xi: xtreg globalCtr_share `treatment_related' `all_controls'  if  WithGlobal==1  , re cluster(sessioncode)
outreg2 using `filename' ,  append `settings'  ctitle("LG Global contribution % in round t+1")

* LG, Global contribution %, controling club size 
* HighC has sig positive impact, but size is related with the treatment variables 
// xi: xtreg globalCtr_share `treatment_related' `all_controls' clubsize if  WithGlobal==1 &  subsessionperiod==1 , re cluster(sessioncode)
// outreg2 using `filename' ,  append `settings'  ctitle("LG Global contribution % in round 1")
//
// xi: xtreg globalCtr_share `treatment_related' `all_controls'  clubsize if  WithGlobal==1  , re cluster(sessioncode)
// outreg2 using `filename' ,  append `settings'  ctitle("LG Global contribution % in round t+1")

* LG, Global contribution % , members only 
xi: xtreg globalCtr_share `treatment_related' `all_controls' if  WithGlobal==1 &  subsessionperiod==1 & playerjoin_club==1, re cluster(sessioncode)
outreg2 using `filename' ,  append `settings'  ctitle("LG Global contribution % in round 1, members")

xi: xtreg globalCtr_share `treatment_related' `all_controls'  if  WithGlobal==1 & playerjoin_club==1  , re cluster(sessioncode)
outreg2 using `filename' ,  append `settings'  ctitle("LG Global contribution % in round t+1, members")

********************************************************
***** Check using OLS
********************************************************

local filename "$outputfolder\05-RegressionAll-v4-OLS-Main-Treatments-`rounds'.xls"

xi: reg round1_totCtr_share hetero `all_controls' if  WithGlobal==0 &  subsessionperiod==1 ,vce(cluster sessioncode)
outreg2 using `filename' ,  replace `settings'  ctitle("L Contribution % in round 1")

xi: reg share_totalContr hetero `all_controls'  if  WithGlobal==0  , vce(cluster sessioncode)
outreg2 using `filename' ,  append `settings'  ctitle("Contribution % in round t+1")

* LG, join 
xi: reg round1_join `treatment_related' `all_controls' if  WithGlobal==1 &  subsessionperiod==1 , vce(cluster sessioncode)
outreg2 using `filename' ,  append `settings'  ctitle("LG join in round 1")

xi: reg playerjoin_club `treatment_related' `all_controls'  if  WithGlobal==1  , vce(cluster sessioncode)
outreg2 using `filename' ,  append `settings'  ctitle("LG join in round t+1")

* LG, Global contribution % 
xi: reg globalCtr_share `treatment_related' `all_controls' if  WithGlobal==1 &  subsessionperiod==1 , vce(cluster sessioncode)
outreg2 using `filename' ,  append `settings'  ctitle("LG Global contribution % in round 1")

xi: reg globalCtr_share `treatment_related' `all_controls'  if  WithGlobal==1  , vce(cluster sessioncode)
outreg2 using `filename' ,  append `settings'  ctitle("LG Global contribution % in round t+1")

* LG, Global contribution % , members only 
xi: reg globalCtr_share `treatment_related' `all_controls' if  WithGlobal==1 &  subsessionperiod==1 & playerjoin_club==1, vce(cluster sessioncode)
outreg2 using `filename' ,  append `settings'  ctitle("LG Global contribution % in round 1, members")

xi: reg globalCtr_share `treatment_related' `all_controls'  if  WithGlobal==1 & playerjoin_club==1  , vce(cluster sessioncode)
outreg2 using `filename' ,  append `settings'  ctitle("LG Global contribution % in round t+1, members")



*******end of 4 Main variables: L, Contribution %
